...
class PerformanceTest {
  ...
  static Future<void> testJSONParsing() async {
    ...
    final sampleEventData = {
      ...
      'properties': {
        'part': {
          ...
          'type': 'text',
          'text':
              'This is a sample streaming text that would be received from the server during a typical Space session.',
        }
      }
    };
    ...
    print(
        '📊 [PerformanceTest] Full JSON parsing: ${fullParsingTime.toStringAsFixed(2)}μs per event');
    ...
    print(
        '📊 [PerformanceTest] Fast path parsing: ${fastPathTime.toStringAsFixed(2)}μs per event');

    final improvement =
        ((fullParsingTime - fastPathTime) / fullParsingTime * 100);
    print(
        '📊 [PerformanceTest] Fast path improvement: ${improvement.toStringAsFixed(1)}%');
  }
  ...
  static Future<void> testDebouncedUpdates() async {
    ...
    print(
        '   Avg latency: ${immediateReport.averageLatencyMs.toStringAsFixed(2)}ms');
    print(
        '   Update frequency: ${immediateReport.averageUpdateFrequency.toStringAsFixed(1)}/sec');
    ...
    print(
        '   Avg latency: ${debouncedReport.averageLatencyMs.toStringAsFixed(2)}ms');
    print(
        '   Update frequency: ${debouncedReport.averageUpdateFrequency.toStringAsFixed(1)}/sec');

    final timeImprovement =
        ((immediateTime - debouncedTime) / immediateTime * 100);
    print(
        '📊 [PerformanceTest] Debouncing improvement: ${timeImprovement.toStringAsFixed(1)}%');
  }
